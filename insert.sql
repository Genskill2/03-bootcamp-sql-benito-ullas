...
--select books.title,publisher.name from books,publisher where publisher.id = books.publisher;
...
/*
select books.title, publisher.name, group_concat(subjects.name)
from books, books_subjects, subjects, publisher 
where books_subjects.book = books.id
and  subjects.id = books_subjects.subject
and books.publisher = publisher.id
